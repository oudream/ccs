...
namespace CCS.DB
{
    public class CsDBOracle : CsIDBConnection
    {
        ...
        public CsDBOracle(string constring, string ConType)
        {
            try
            {
        ...
        public bool ExcuteDataForManySql(string[] sqls)
        {
            lock (this.thislock)
            {
                if (this.IsOpen())
                {
                    this.oraclcmd = this.oracleCon.CreateCommand();
                    OracleTransaction transaction = this.oracleCon.BeginTransaction(IsolationLevel.ReadCommitted);
                    try
                    {
        ...
        public object GetDataFirst(string sql, int col)
        {
            ...
            lock (this.thislock)
            {
                ...
                try
                {
                    ...
                    {
                        CsInterinfo.OutInfoPrompt("使用ExecuteReader读取数据失败!" + exception.Message);
                    }
                    return str;
                }
                finally
